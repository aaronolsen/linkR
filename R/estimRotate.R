estimRotate <- function(coor, coor.align = NULL, max.iter.use = 20, max.pair.use = 20, 
	unique.pairs = FALSE, wt = 'angle'){
	...
	if(FALSE){
		...
		svg.arrows(rbind(cor_avg, cor_avg + 2*aor_avg), col='purple')

		svg.points(cor_mat, col='red', cex=2)
		for(i in 1:nrow(cor_mat)) svg.arrows(rbind(cor_mat[i,], cor_mat[i,] + 1*aor_mat[i,]), col='red')

		svg.points(c(0.1,0.3,0.5), col='orange', cex=3)
		
		svg.arrows(rbind(c(0.1,0.3,0.5), c(0.1,0.3,0.5)+2*uvector(c(0,0,1))), col='orange')
		#svg.arrows(rbind(c(0.1,0.3,0.5), c(0.1,0.3,0.5)+2*uvector(c(1,0,0))), col='orange')
