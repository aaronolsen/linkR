drawMechanism2 <- function(mechanism, method = "svgViewR", file = NULL, animate = TRUE, 
	...
	if(is.null(mechanism[['joint.coor.anim']])){
		joints <- array(mechanism[['joint.coor']], dim=c(dim(mechanism[['joint.coor']])[1], dim(mechanism[['joint.coor']])[2], 1, 1))
	}else{
		...
		for(i in 1:length(mechanism[['joint.cons']])){
			...
			for(j in 1:num_iter){
				for(k in seq(1, n_vectors*2, by=2)){
					cons_vec1[[i]][k, , j] <- mechanism[['joint.coor.anim']][i, , j, 1]
					cons_vec1[[i]][k+1, 1:3, j] <- mechanism[['joint.coor.anim']][i, , j, 1] + 
						arrow_len_part*uvector(mechanism[['joint.cons.anim']][[i]][(k+1)/2, , j, 1])
					if(debug){
						cons_vec2[[i]][k, , j] <- mechanism[['joint.coor.anim']][i, , j, 2]
						cons_vec2[[i]][k+1, 1:3, j] <- mechanism[['joint.coor.anim']][i, , j, 2] + 
							0.8*arrow_len_part*uvector(mechanism[['joint.cons.anim']][[i]][(k+1)/2, , j, 2])
					}
				}
			}
		}
