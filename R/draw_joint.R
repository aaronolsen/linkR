draw_joint <- function(file, animate.joint, fixed, fixed.lcs, col=c('red', 'green', 'blue'), 
	cons.col=c('purple', 'MediumPurple', 'Magenta'), cex=c(3,7), lwd=1, draw.lcs = TRUE, 
	...
	for(i in 1:length(animate.joint)){
		...
		if(dof[1] > 0){
			...
			for(j in 1:dof[1]){
				if(animate){
					...
					svg.arrows(arrow_arr, lwd=lwd, col=cons.col[j], len=head.len, opacity=opacity)
				}else{
					for(iter in 1:n_iter){
						svg.arrows(rbind(jt_cons[iter, 1:3], jt_cons[iter, 1:3]+0.75*arrow.len*uvector(jt_cons[iter, (j*3+1):(j*3+3)])), 
							lwd=lwd, col=cons.col[j], len=head.len, opacity=opacity)
					}
				}
			}

			# Draw point at center of rotation			
			svg.pointsC(array(t(jt_cons[, 1:3]), dim=c(1,3,nrow(jt_cons))), cex=cex[1], col='purple')
		...
		if(dof[2] > 0){
			...
			for(j in 1:dof[2]){
				...
				svg.arrows(arrow_arr, lwd=1, col=cons.col[j], len=head.len, opacity=opacity)
			}
		}
	}
