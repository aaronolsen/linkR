		...
		while(it < 100){
			...
			for(i in joint_path_idx){
				...
				for(j in 1:length(adj_joints)){
					...
					if(print.progress) cat(paste0(paste0(joint_path[[add_at]], collapse=','), ' (', paste0(body.names[body_path[[add_at]]], collapse=','), ')'))
					...
					if(print.progress) cat(paste0(': ', jt_str[['t']], ' [', jt_str[['f']], ']'))
			...
			if(!something_changed){
				if(print.progress) cat(' No change')
				break
			}
